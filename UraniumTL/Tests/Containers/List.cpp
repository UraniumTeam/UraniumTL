...
TEST(List, CopyConstruct)
{
    ...
    for (UN::USize i = 0; i < lst1.Size(); ++i)
    {
...
TEST(List, Indexing)
{
    List<UN::USize> lst = { 0, 0, 0 };
    lst[1]              = 1;
    lst[2]              = 2;
    ASSERT_EQ(lst.Size(), 3);
    for (UN::USize i = 0; i < lst.Size(); ++i)
    {
        EXPECT_EQ(lst[i], i);
    }
}
...
TEST(List, Append)
{
    ...
    for (UN::USize i = 0; i < lst1.Size(); ++i)
    {
        EXPECT_EQ(UN::StringSlice(lst1[i]), UN::StringSlice(lst2[i]));
    }
}
...
TEST(List, Remove)
{
    auto allocatedBefore = UN::SystemAllocator::Get()->AllocationCount();
    {
        auto lst = List<UN::String>{}.Append("1").Append("2").Append("3").Append("4").Append("5").Append("6");
        lst.Remove("2");
        EXPECT_EQ(lst.Size(), 5);
        EXPECT_EQ(lst[0], "1");
        EXPECT_EQ(lst[1], "3");
    ...
    EXPECT_EQ(UN::SystemAllocator::Get()->AllocationCount(), allocatedBefore);
}
